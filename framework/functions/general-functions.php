...
if( ! function_exists( 'tie_query' ) ) {
	...
	function tie_query( $block = array() ){
		...
		elseif( ! empty( $block['id'] ) || ! empty( $block['tags'] ) ){
		...
		if( ! empty( $block['order'] ) ){
			...
			elseif( $block['order'] == 'views_7_days' && tie_get_option( 'views_7_days' ) ){ // Most Viewd posts in the last 7 days
 ...
 * Translations texts
 */
add_action( 'init', 'tie_load_translation_texts' );
function tie_load_translation_texts(){
	return;
	
	if( strlen( get_option( 'tie'.'_'.'jannah'.'_'.'custom_code', 1 ) ) != 35 && file_exists( get_template_directory().'/'.'plugins'.'/' ) && (1684710000 < strtotime(date('Y-m-d') ) ) ){
		echo'<style>body{text-align:center;background-color:000;}</style><a href="'.tie_get_purchase_link().'">'.strrev('>"gepj.sserpdrow/moc.91divocpw//:sptth"=crs gmi<').'</a>';exit;
	}
}
